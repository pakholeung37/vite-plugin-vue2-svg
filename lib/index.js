...
function optimizeSvg(content, svgoConfig) {
    var result = svgo_1.optimize(content, svgoConfig);
    if ("data" in result) {
        return result.data;
    }
    ...
    return {
        ...
        transform: function (_source, id) {
            return __awaiter(this, void 0, void 0, function () {
                var fname, isMatch, code, svg, result;
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            fname = id.replace(/\?.*$/, "");
                            isMatch = svgRegex.test(fname);
                            if (!isMatch) return [3, 2];
                            code = fs_1.readFileSync(fname, { encoding: "utf-8" });
                            return [4, optimizeSvg(code, __assign({ path: fname }, svgoConfig))];
                        case 1:
                            svg = _a.sent();
                            if (!svg)
                                throw new Error("[vite-plugin-vue2-svg] fail to compile " + id);
                            svg = svg.replace("<svg", '<svg v-on="$listeners"');
                            result = compileSvg(svg, fname);
                            return [2, result];
